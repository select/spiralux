    ...
    >
      ...
      <div class="absolute bottom-3 right-3 flex flex-col items-end gap-2">
        ...
        <div class="flex items-center gap-1.5 bg-base/70 backdrop-blur-md border border-border/60 rounded-xl px-2 py-1.5 shadow-[0_4px_16px_var(--color-shadow)]">
          ...
          <button
            class="px-2.5 py-1.5 rounded-lg text-xs text-secondary hover:text-primary hover:bg-elevated/60 transition-all duration-150 cursor-pointer active:scale-95"
            title="Clear (C)"
            @click="clearCanvas"
          >🗑</button>
          <button
        ...
        <div class="flex items-center gap-1.5 bg-base/70 backdrop-blur-md border border-border/60 rounded-xl px-2 py-1.5 shadow-[0_4px_16px_var(--color-shadow)]">
          <button
            ...
            :class="liveMode
              ...
              : 'text-muted hover:text-secondary border border-transparent hover:border-border/60'"
            title="Live preview mode (L) — redraws 10k steps on every slider change"
          ...
          <button
            ...
            :class="showMachine
              ? 'bg-accent/15 text-accent border border-accent/30'
              : 'text-muted hover:text-secondary border border-transparent hover:border-border/60'"
            title="Toggle machine diagram (M)"
