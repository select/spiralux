...
/** Generate a URL-safe slug from a label: "3-Lobe Trefoil" → "3-lobe-trefoil" */
...
/** Look up a target by its slug (tries known targets first, then derives from file). */
export function findTargetBySlug(slug: string, all: TargetInfo[]): TargetInfo | undefined {
  return all.find(t => toSlug(t.label) === slug);
}
